...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            manager.save()
            self.stdout.write(
                self.style.SUCCESS(f'Создан менеджер: username=manager, password=manager123')
            )
        else:
        ...
        if created:
            ...
            customer.save()
            self.stdout.write(
                self.style.SUCCESS(f'Создан покупатель: username=customer, password=customer123')
            )
        else:
        ...
        # Создаем категории
        categories_data = [
            {'name': 'Мужская одежда', 'slug': 'mens-clothing', 'description': 'Стильная мужская одежда'},
            {'name': 'Женская одежда', 'slug': 'womens-clothing', 'description': 'Модная женская одежда'},
            {'name': 'Обувь', 'slug': 'shoes', 'description': 'Качественная обувь'},
            {'name': 'Аксессуары', 'slug': 'accessories', 'description': 'Стильные аксессуары'},
        ]
