...
from .forms import CustomUserCreationForm, ProductForm, CategoryForm, CheckoutForm, UserRoleForm
...
def dashboard(request):
    ...
    for i in range(7):
        ...
        sales_by_day.append({
            ...
            'revenue': day_orders.aggregate(Sum('total_price'))['total_price__sum'] or 0
        })
    ...
    if prev_month_revenue > 0:
        revenue_growth = round(((recent_revenue - prev_month_revenue) / prev_month_revenue) * 100, 1)
    
    if prev_month_count > 0:
        orders_growth = round(((recent_orders_count - prev_month_count) / prev_month_count) * 100, 1)
...
def admin_product_delete(request, product_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('shop:admin_products')
    return render(request, 'shop/admin_product_delete.html', {
        'product': product,
        'base_template': get_base_template(request.user),
    })
...
def admin_categories(request):
    categories = Category.objects.all()
    return render(request, 'shop/admin_categories.html', {
        'categories': categories,
        'base_template': get_base_template(request.user),
    })
...
def admin_category_create(request):
    if request.method == 'POST':
        ...
        form = CategoryForm()
    return render(request, 'shop/admin_category_form.html', {
        'form': form, 
...
def admin_user_role(request, user_id):
    ...
    return render(request, 'shop/admin_user_role.html', {
        'form': form, 
        'user': user,
        'base_template': get_base_template(request.user),
    })
